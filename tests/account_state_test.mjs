...
/* 14. an unbound container is off with no account until explicitly enabled */
check("unbound container inactive", am.isActive("c9"), false);
check("unbound container no account", am.getActive("c9"), undefined);
am.setActive(true, "c9");
check("enabled container active", am.isActive("c9"), true);
...
/* 18. legacy single-state data migrates to the default container only */
reset({ state: true, accounts: [acc("a@x", true)] });
am = new AccountManager();
await am.restore();
check("legacy migrated enabled", am.isActive(), true);
check("legacy migrated account", am.getActive()?.username(), "a@x");
check("legacy other container off", am.isActive("cX"), false);
check("legacy other container no account", am.getActive("cX"), undefined);
