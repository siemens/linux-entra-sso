...
export class AccountManager {
    ...
    #bindings = new Map();
    /* read-only default-container fallback (SSO on, nothing selected) */
    #fallback = { sso: new SsoStateMachine(DEFAULT_STORE), username: null };
    /* read-only fallback for other containers (SSO off, nothing selected) */
    #inactive = { sso: new SsoStateMachine(DEFAULT_STORE), username: null };
    /* in-flight token requests, keyed by username, to dedup concurrent calls */
    #token_requests = new Map();

    constructor() {
        this.#inactive.sso.log_out();
    }
    ...
    /*
     * Resolve the binding for a container. Only the global/default container
     * has an implicit "SSO on, nothing selected" default; other containers
     * stay off with no account until the user explicitly enables them.
     */
    #resolve(store = DEFAULT_STORE) {
        return (
            this.#bindings.get(store) ??
            (store === DEFAULT_STORE ? this.#fallback : this.#inactive)
        );
    }
    ...
    /* Materialize an explicit binding for a container, seeded from its default. */
    #ensure(store) {
